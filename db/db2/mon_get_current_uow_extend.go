...
//获取当前正在执行的uow事务，只包含未做提交的所有事务并且包含相关agent信息
...
func NewMonGetCurUowExtend() *MonGetCurUowExtend {
	...
	m.tabname = "(select uow.*,agent.AGENT_STATE,agent.EVENT_TYPE,agent.EVENT_OBJECT,agent.EVENT_STATE,agent.REQUEST_TYPE,agent.NESTING_LEVEL,agent.EXECUTABLE_ID,agent.UTILITY_INVOCATION_ID from table(MON_GET_UNIT_OF_WORK(null,-1)) as uow left join table(MON_GET_AGENT('','',cast(NULL as bigint), -1)) agent on uow.APPLICATION_HANDLE=agent.APPLICATION_HANDLE and uow.UOW_ID=agent.UOW_ID  and uow.UOW_STOP_TIME is null and agent.AGENT_TYPE='COORDINATOR' )"
